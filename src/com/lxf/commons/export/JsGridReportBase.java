...
package com.lxf.commons.export;
import java.io.File;
...
public class JsGridReportBase {
	...
	private String MODULE_PATH = Log.getRootPath()+"\\util\\module.xls";//模板路径
	...
	private HashMap<Integer, Integer> heights = null;
	
	private HashMap<Integer, Integer> widths = null;
	
	private boolean showSubTitle = true;
	
	private boolean globalWrapText = false;
	...
	public HSSFWorkbook writeSheet(HSSFWorkbook wb, String title, HashMap<String, HSSFCellStyle> styles, String creator, TableData tableData) throws Exception {
		...
		if(null != heights){
			for(Integer i : heights.keySet()){
				sheet.getRow(i).setHeight(heights.get(i).shortValue());
			}
		}
		
		if(null != widths){
			for(Integer j : widths.keySet()){
	...
	/**
	 * @return the globalWrapText
	 */
	public boolean isGlobalWrapText() {
		return globalWrapText;
	}

	/**
	 * @param globalWrapText the globalWrapText to set
	 */
	public void setGlobalWrapText(boolean globalWrapText) {
		this.globalWrapText = globalWrapText;
	}
	...
	/**
	 * @param mODULE_PATH the mODULE_PATH to set
	 */
	 ...
	 * @return the widths
	 */
	public HashMap<Integer, Integer> getWidths() {
		return widths;
	}
	 ...
	 */
	public void setWidths(HashMap<Integer, Integer> widths) {
		this.widths = widths;
	}

	/**
	 * @return the showSubTitle
	 */
