...
public class OrdersMgrAction extends OrdersMgrActionBean{
	...
	public String list(){
		String filedTitle = "订单号,供应商,联系人,联系方式,下单时间,进货种类,退货种类,总计";
		colsTable = "ORDER_ID,C_NAME,C_CONTACT,C_TEL,ADDTIME,JH_NUM,TD_NUM,TOTAL_NUM";
        //字段是否显示
        boolean [] isShow = {true,true,true,true,true,true,true,true};
        //当前列是否自动排序
        boolean [] isFiledSort = {true,true,true,true,true,true,true,true};
        //当前列是否冻结
        boolean [] frozen = {false,false,false,false,false,false,false,false};
        //单元格是否加格式化函数
        boolean [] isFormatColumn = {false,false,false,false,false,true,true,true};
        //单元格是否加样式函数
        boolean [] isStylerColumn = {false,false,false,false,false,false,false,false};
        String widths[] = {"120", "120","120","120","120","120","120","120"};
	...
	public String getReturnOrdersProductById(){
		String filedTitle = "pid,件号,产品,退货价,退货量,总价";
		colsTable = "p_id,p_part_no,p_name,p_price,p_num,totalcost";
		//字段是否显示
		boolean [] isShow = {false,true,true,true,true,true};
		//当前列是否自动排序
		...
		String widths[] = {"120","120", "120","120","120","120"};
		String aligns[] = {"center","center","center", "center", "center", "center"};
		operationField = "{title:'退货信息',colspan:5}";
		this.formatEasyuiFields(false, filedTitle, colsTable, isShow, frozen, isFiledSort,isFormatColumn, isStylerColumn,isEditor,isRequired,editorType, widths, aligns);// 如果第二个参数为null，默认为全排序
		return DEFAULT;
	}
	 ...
	 */
	@SuppressWarnings({ "rawtypes", "unchecked" })
	public void getReturnOrdersProductByIdAjax() throws Exception{
		pager.setPageSize(30);	
		List result = orderMgrService.getReturnOrdersProductById(pager, sord, sidx,orderIds);
		super.outJsonData(result, null, pager, colsTable);
	}
	...
	public void returnOrdersProducts(){
		orderMgrService.returnOrdersProducts(orderIds,p_part_no);
	}
