...
﻿	//alert(document.getElementsByTagName("head")[0].innerHTML);
	...
	for(var i=0;i<tagaArry.length;i++)
	{
		...
		if(oldTitle!=null && oldTitle!="")
		{
			...
			{			
				setTipLocation(window.event || arguments[0]);
			}
...
function setTipLocation(e)
{
	...
	*/
	...
	//取得tooltip对象
	var tipbar = $$$(_BarIdName);
	//alert(tipbar.offsetHeight);
	//alert(intY+":"+document.body.clientHeight);
	
	if(document.body.clientHeight-(tipbar.offsetHeight+intY +10)>50)
	{
		tipbar.style.top = (intY+20)+"px";
	}
	else
	{
		tipbar.style.top = (intY-2-tipbar.offsetHeight)+"px";
	}
	
	
	
	if(document.body.scrollWidth-(intX +10)>300)
	{
		tipbar.style.left = (intX +10)+"px";
	}
	else
	{
		tipbar.style.left = (intX -310)+"px";
	}
}
...
{		
	...
	linkstyle.setAttribute("href","../css/tooltip.css");
